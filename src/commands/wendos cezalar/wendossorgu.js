...
module.exports = {
  ...
  run: async (client, message, args, embed) => {
    ...
    message.channel.send({ embeds: [embed.setDescription(`#${data.id} ${data.active ? emojis.red : emojis.green} **[${data.type}]** <@${data.userID}> üyesi, ${moment(data.date).format("LLL")} tarihinde, <@${data.staff}> tarafından, \`${data.reason}\` nedeniyle, ${data.type.toLowerCase().replace("-", " ")} cezası almış.`)] });
  },
};