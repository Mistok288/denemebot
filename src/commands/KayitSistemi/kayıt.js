...
run: async (client, message, args, embed, prefix) => { 
    ...
    {
    message.react(red)
    message.reply({ content:`> \`${prefix}kayıt <@wendos/ID> <Isim/yaş>\``}).then((e) => setTimeout(() => { e.delete(); }, 5000)); 
    return }
...
.setDescription(`
> **${uye.toString()} kullanıcısının ismini \`"${setName}"\` olarak degiştirildi**
`)
...
if(msg) msg.delete();
 button.channel.send({ embeds: [ozie], components: [], ephemeral: false});
    ...
    if(ayar.chatChannel && client.channels.cache.has(ayar.chatChannel)) client.channels.cache.get(ayar.chatChannel).send({ content:`> **${uye}** <t:${Math.floor(Date.now() / 1000)}:R> kayıt oldu , Aramıza hoşgeldin dostum.`}).then((e) => setTimeout(() => { e.delete(); }, 20000));
