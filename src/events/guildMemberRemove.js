...
module.exports = async (member) => {
  ...
  if (!inviteMemberData) {
    channel.wsend({ content: `\`${member.user.tag}\` **<t:${Math.floor(Date.now() / 1000)}:R>** Sunucudan çıktı davet edilen bulanamadı`});
  } else {
    ...
    channel.wsend({ content:`\`${member.user.tag}\` **<t:${Math.floor(Date.now() / 1000)}:R>** sunucudan çıktı. ${inviter.tag} tarafından davet edilmişti. (**${total}** davet)`});
