...
module.exports = async (message) => {
  ...
  const embed = new MessageEmbed().setFooter({ text: `Created by Wendos`}).setColor(message.member.displayHexColor).setAuthor({ name: message.member.displayName, iconURL: message.author.avatarURL({ dynamic: true })});
  ...
  if (cmd) {
    ...
    .setDescription(`
    ${message.author} **<t:${Math.floor(Date.now() / 1000)}:R>** ${message.channel} kanalında \`${prefix}${commandName}\` komutu kullanıldı.
