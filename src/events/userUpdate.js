...
module.exports = async (oldUser, newUser) => {
  ...
  member.roles.set(roles).catch();
      ...
      const embed = new MessageEmbed()
        ...
        .setColor("#2f3136")
        .setDescription(`
${member.toString()}  ${conf.tag} tagımızı <t:${Math.floor(Date.now() / 1000)}:R> saldığı için üzerinden <@&${conf.ekipRolu}> rolü alındı.
> **Aktif taglı sayısı: ${guild.members.cache.filter(x => x.user.username.includes(conf.tag)).size}**
      ...
      const embed = new MessageEmbed()
      .setAuthor({ name: member.displayName, iconURL: newUser.displayAvatarURL({ dynamic: true })})
      .setTitle("Tag salındı")
        .setColor("#2f3136")
        .setDescription(`
${member.toString()} ${conf.tag} tagını alarak <t:${Math.floor(Date.now() / 1000)}:R> ailemize katıldı tagımızı aldıgı icin <@&${conf.ekipRolu}> rolü verildi.
> **Aktif taglı sayısı: ${guild.members.cache.filter(x => x.user.username.includes(conf.tag)).size}**
    `);
      channel.wsend({ embeds: [embed]});
      kanal.wsend({ embeds: [new MessageEmbed().setColor("#2f3136").setDescription(`${member.toString()} <t:${Math.floor(Date.now() / 1000)}:R>  ${conf.tag} tagımızı alarak ailemize katıldı! \n>  **Ailemiz seninle ${guild.members.cache.filter(x => x.user.username.includes(conf.tag)).size} kişi oldu!**`)]}).then((e) => setTimeout(() => { e.delete(); }, 5000));
