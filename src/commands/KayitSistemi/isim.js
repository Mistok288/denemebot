...
run: async (client, message, args, prefix) => { 
    ...
    {
    message.react(red)
    message.reply({ content:`\`.isim <@wendos/ID> <Isim> <Yas>\``}).then((e) => setTimeout(() => { e.delete(); }, 5000)); 
    return }
    ...
    {
    message.react(red)
    message.reply({ content:`\`${prefix}isim <@wendos/ID> <Isim>\``}).then((e) => setTimeout(() => { e.delete(); }, 5000)); 
    return }
    if(!yaş) 
